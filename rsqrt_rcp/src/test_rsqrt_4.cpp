...
//  Time-stamp: "Modified on 28-April-2016 at 17:18:19 by jearnold on olhswep03.cern.ch"

//  The output file consists of 2^24 single-precision floating-point numbers corresponding to the values of various forms of the
//  "Ofast rsqrt" function for each single-precision floating-point number in [1,4).
...
main(int argc, char* argv[]) {
    ...
    while (current_arg <= last_arg) {
        ...
        for (unsigned int i = 0; i < sizeof(arg_list) / sizeof(float); i++) {
            data_buffer[ index++ ] = rsqrt_Ofast(arg_list[ i ]);
        }
#else
        __m256 results;
#if !defined(IMPROVED)
