...
//  Time-stamp: "Modified on 29-October-2015 at 15:12:15 by jeff on Jeffs-MacBook-Pro"

//  Generate a datafile of the results of 1.0/std::sqrt(x) for x in [1,4).  These are the "reference" values.

//  The output file consists of 2^24 double-precision floating-point numbers corresponding to the values of 1.0/std::sqrt(x) for
//  each single-precision floating-point number in [1,4).  E.g., file_value = 1.0/std::sqrt((double)x) for float x in [1,4).
...
main(int argc, char* argv[]) {
    ...
    //  There are 8 * ( 2 * 2^23) = 134217728 bytes of data in the file for a file size of 134217728 bytes.
    ...
    while (current_arg <= last_arg) {
        ...
        __m256d results = _mm256_div_pd(ones, _mm256_sqrt_pd(arg_list));
